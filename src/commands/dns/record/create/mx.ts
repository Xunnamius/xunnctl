    ...
    handler: await withGlobalOptionsHandling<CustomCliArguments>(
      ...
      async function ({
        ...
        taskManager.add([
          ...
          withStandardListrTaskConfig({
            initialTitle: `Creating "MX" resource records in selected Cloudflare zones...`,
            apiCallerFactory: makeCloudflareApiCaller,
            configPath,
            debug,
            async callback({ api, taskLogger, thisTask: zoneTask }) {
              const count = Object.keys(results.cfZoneApexIds).length;
              zoneTask.title = `Creating "MX" resource records in ${count} Cloudflare zones...`;
              ...
              try {
                ...
                zoneTask.title = `Created ${totalRecordCount} "MX" resource record${totalRecordCount === 1 ? '' : 's'} in ${count} Cloudflare apex domain${count === 1 ? '' : 's'}`;
              } catch (error) {
                throw new TaskError(
          ...
          withStandardListrTaskConfig({
            initialTitle: `Creating "MX" resource records in selected DigitalOcean zones...`,
            apiCallerFactory: makeDigitalOceanApiCaller,
            configPath,
            debug,
            async callback({ api, taskLogger, thisTask: zoneTask }) {
              const count = Object.keys(results.doZoneApices).length;
              zoneTask.title = `Creating "MX" resource records in ${count} DigitalOcean zones...`;
              ...
              try {
                ...
                await Promise.all(
                  results.doZoneApices.map(async (zoneName) => {
                    taskLogger('creating "MX" record for %O', zoneName);
                ...
                zoneTask.title = `Created ${totalRecordCount} "MX" resource record${totalRecordCount === 1 ? '' : 's'} in ${count} DigitalOcean apex domain${count === 1 ? '' : 's'}`;
              } catch (error) {
                throw new TaskError(
                  'failed to create "MX" resource record in DigitalOcean account',
                  { cause: error }
                );
              }
            }
